...
import { FILE_BASE } from '../services/api/index';
...
export const normalizeImageUrl = (url: string | undefined | null): string => {
  ...
  // 文件路径（以 /files/ 开头）- 需要添加后端服务器地址
  if (url.startsWith('/files/')) {
    return `${FILE_BASE}${url}`;
  }
  
  // 文件路径（以 / 开头，但不是 base64）
  if (url.startsWith('/') && !url.startsWith('/9j/') && !url.startsWith('/+')) {
    return `${FILE_BASE}${url}`;
  }
...
export const getThumbnailUrl = (originalUrl: string | undefined | null): string => {
  ...
  // 移除可能存在的后端服务器前缀，获取纯路径
  let cleanUrl = originalUrl;
  if (originalUrl.startsWith('http://127.0.0.1:8765')) {
    cleanUrl = originalUrl.replace('http://127.0.0.1:8765', '');
  }
  ...
  return `${FILE_BASE}/files/thumbnails/${normalizedDirName}_${nameWithoutExt}_thumb.jpg`;
};
